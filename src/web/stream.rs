...
use actix_web::{web::Bytes, HttpResponse, Result};
use futures_util::stream::Stream;
use std::sync::Arc;
use tokio::sync::Mutex;
use tokio::time::{interval, Duration};

pub async fn mjpeg_stream(camera: Arc<Mutex<CameraManager>>) -> Result<HttpResponse> {
...
fn create_mjpeg_stream(
    ...
    futures_util::stream::unfold((camera, interval), |(camera, mut interval)| async move {
        interval.tick().await;
