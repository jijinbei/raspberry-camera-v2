...
#[get("/capture")]
pub async fn capture(camera: web::Data<Arc<Mutex<CameraManager>>>) -> Result<HttpResponse> {
    let mut camera = camera.lock().await;

    match camera.capture_photo().await {
        Ok(jpeg_data) => Ok(HttpResponse::Ok()
...
#[get("/stream")]
pub async fn stream(camera: web::Data<Arc<Mutex<CameraManager>>>) -> Result<HttpResponse> {
